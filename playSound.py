...
if __name__ == "__main__":
    ...
    audio, sample_rate = librosa.load(audio_path, sr=16000)  # resample to 16 kHz
    print(f"[Main] Loaded {audio.shape[0]} samples @ {sample_rate} Hz")

    # Start playback in a separate process
    PlaySound(audio, sample_rate)

    # Continue doing other work while sound plays
    for i in range(5):
